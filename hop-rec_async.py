...
class HopRec():
    ...
    def train(self, sample_times: int, learning_rate: float, walk_steps: int, lambda_: float, negative_sample_times: int, save_path: str, process_num: int = 1):
        ...
        start = time.time()
        tasks = [self.train_worker(i) for i in range(self.total_sample_times)]
    ...
    async def train_worker(self, _: int):
        learning_rate_origin = self.learning_rate
        global training_learning_rateＦ
        ...
        for step in range(1, self.walk_steps+1):
            ...
            self.updateFBPRPair(user, pos_item,
                                training_learning_rate/step, margin/step, self.lambda_, self.negative_sample_times)
        # update learning rate
        current_sample += 1 
        training_learning_rate = learning_rate_origin * \
            (1 - current_sample / self.total_sample_times)
        if current_sample % MONITOR == 0:
            sys.stdout.flush()
            sys.stdout.write("\b" * 100)
            sys.stdout.write(
                f"Learing rate: {training_learning_rate:.6f}\t{current_sample/self.total_sample_times*100:6.3f}%")
