...
def predict():
    ...
    for usr in tqdm(usr_to_idx):
        ...
        predict_file.write(f"{usr[2:]},")
        for k, candidate in enumerate(candidates[:TOP_K]):
            predict_file.write(f"{candidate['itm'][2:]} ")
        predict_file.write(f"\n")
        predict_file.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument("-graph", "--graph",
                        help="Specify graph path", default="net.dat")
    parser.add_argument("-model", "--model",
                        help="Specify model path", default="rep_line2.txt")
    parser.add_argument(
